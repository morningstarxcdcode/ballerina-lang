...
public class TypesFactory {
    ...
    private BTypeSymbol createSyntheticTypeSymbol(BType bType) {
        // Create a synthetic type symbol for types that don't have one
        // This prevents NPE when accessing symbol properties
        return new BTypeSymbol(
            SymTag.TYPE,
            Flags.ANONYMOUS,
            Names.fromString("synthetic_" + bType.getKind()),
            PackageID.ANNOTATIONS, // Use annotations package as it's always available
            bType,
            null, // No owner for synthetic symbols
            symTable.builtinPos, // Use builtin position
    ...
    /**
     * Validates if a BType has a valid type symbol reference.
     * This prevents NPE when accessing symbol properties.
     *
     * @param bType The BType to validate
     * @return true if the type has a valid symbol, false otherwise
     */
    ...
    /**
     * Ensures that a BType has a valid type symbol by creating a synthetic one if needed.
     * This method should be called before any semantic API operations that require symbol access.
     *
     * @param bType The BType to ensure has a symbol
     */
    ...
    /**
     * Safely gets the type descriptor with comprehensive null safety checks.
     * This is a null-safe wrapper around getTypeDescriptor that handles all edge cases.
     *
     * @param bType The BType to get descriptor for
     * @param tSymbol The associated type symbol (can be null)
     * @return TypeSymbol or null if type cannot be resolved
     */
    public TypeSymbol getTypeDescriptorSafe(BType bType, BSymbol tSymbol) {
        try {
            ...
            // Ensure type symbol exists for structured types
            ensureTypeSymbol(bType);

            // Get the type descriptor with error handling
            return getTypeDescriptor(bType, tSymbol);

        } catch (Exception e) {
            // Log error for debugging but don't crash
            // This handles cases where type creation fails due to incomplete symbol information
            return null;
        }
    }

    /**
     * Validates type parent references and initializes missing ones.
     * This is crucial for anonymous types that may not have proper parent relationships.
     *
     * @param bType The BType to validate and initialize
     * @return true if validation/initialization succeeded, false otherwise
     */
    public boolean validateAndInitializeTypeParents(BType bType) {
        ...
        try {
            // Ensure the type itself has a symbol
            ensureTypeSymbol(bType);

            // For structured types, validate and initialize member types
            switch (bType.getKind()) {
                case RECORD -> {
                    BRecordType recordType = (BRecordType) bType;
                    if (recordType.fields != null) {
                        recordType.fields.values().forEach(field ->
                            ensureTypeSymbol(field.type));
                    }
                    if (recordType.restFieldType != null) {
                        ensureTypeSymbol(recordType.restFieldType);
                    }
                }
                case OBJECT -> {
                ...
                case UNION -> {
                    ...
                    if (unionType.getMemberTypes() != null) {
                        unionType.getMemberTypes().forEach(this::ensureTypeSymbol);
                    }
                }
                case ARRAY -> {
                    BArrayType arrayType = (BArrayType) bType;
                    ensureTypeSymbol(arrayType.getElementType());
                }
                case MAP -> {
                    BMapType mapType = (BMapType) bType;
                    ensureTypeSymbol(mapType.constraint);
                }
                case TABLE -> {
                    BTableType tableType = (BTableType) bType;
                    ensureTypeSymbol(tableType.constraint);
                }
                case TUPLE -> {
                ...
                case STREAM -> {
                    ...
                    ensureTypeSymbol(streamType.constraint);
                }
            }
            ...
            // Validation/initialization failed
            return false;
        }
    }
}
