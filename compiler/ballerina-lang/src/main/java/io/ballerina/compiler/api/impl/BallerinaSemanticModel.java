...
public class BallerinaSemanticModel implements SemanticModel {
    ...
    public Optional<TypeSymbol> type(LineRange range) {
        ...
        return Optional.ofNullable(typesFactory.getTypeDescriptorSafe(node.getBType(), node.getBType() != null ? node.getBType().tsymbol : null));
    }
    ...
    public Optional<TypeSymbol> typeOf(LineRange range) {
        ...
        try {
            ...
            // This handles cases where type descriptors can't be created due to incomplete symbol information
            return Optional.empty();
        }
    }
    ...
    // ===== COMPREHENSIVE NULL SAFETY METHODS =====

    /**
     * Null-safe wrapper for type resolution that provides comprehensive error handling
     * and meaningful error messages instead of NPE crashes.
     *
     * @param bType The BType to resolve
     * @return TypeSymbol or empty Optional if resolution fails
     */
    private Optional<TypeSymbol> resolveTypeSafely(BType bType) {
        ...
        try {
            // Validate and initialize type symbols
            if (!typesFactory.validateAndInitializeTypeParents(bType)) {
                // Log warning but don't crash - type validation failed
                return Optional.empty();
            }

            // Use the safe type descriptor method
    ...
    /**
     * Enhanced typeOf method with comprehensive null safety checks.
     * This method validates inputs, ensures type symbols exist, and provides
     * graceful degradation instead of crashes.
     *
     * @param range The line range to get type for
     * @return TypeSymbol or empty if type cannot be determined
     */
    public Optional<TypeSymbol> typeOfSafe(LineRange range) {
        ...
        try {
            ...
            if (node == null || (!isNonNamedArgExprNode(node) && !isObjectConstructorExpr(node) &&
                    !isAnonFunctionExpr(node))) {
                return Optional.empty();
            }
    ...
    /**
     * Enhanced type method with null safety checks for Node input.
     *
     * @param node The syntax tree node to get type for
     * @return TypeSymbol or empty if type cannot be determined
     */
    public Optional<TypeSymbol> typeSafe(Node node) {
        // Input validation
        if (node == null) {
            return Optional.empty();
        }
        ...
        try {
            Optional<Location> nodeLocation = node.apply(new SyntaxNodeToLocationMapper());
    ...
    /**
     * Enhanced typeOf method with null safety checks for Node input.
     *
     * @param node The syntax tree node to get type for
     * @return TypeSymbol or empty if type cannot be determined
     */
    public Optional<TypeSymbol> typeOfSafe(Node node) {
        // Input validation
        if (node == null) {
            return Optional.empty();
        }
    ...
    /**
     * Comprehensive error handling wrapper for semantic API operations.
     * Provides meaningful error messages and graceful degradation.
     *
     * @param operation The operation to perform
     * @return Result of the operation or empty/null on failure
     */
    ...
    /**
     * Functional interface for semantic operations that may throw exceptions.
     */
    @FunctionalInterface
